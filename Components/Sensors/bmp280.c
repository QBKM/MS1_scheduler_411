...
#define BMP280_I2C_HANDLE	&hi2c1
...
uint8_t bmp280_read_fixed(int32_t *temperature, uint32_t *pressure)
{
	...
	if(HAL_I2C_Mem_Read(BMP280_I2C_HANDLE, BMP280_ADDR<<1, BMP280_REG_PRESS_MSB, 1, data, sizeof(data), TIMEOUT_I2C)) return HAL_ERROR;
...
static uint8_t bmp280_read_register16(uint8_t addr, uint16_t *value)
{
	...
	if (HAL_I2C_Mem_Read(BMP280_I2C_HANDLE, BMP280_ADDR<<1, addr, 1, rx_buff, 2, 5000) == HAL_OK)
	{
...
uint8_t BMP280_Init(void)
{
	...
	if(HAL_I2C_Mem_Read(BMP280_I2C_HANDLE, BMP280_ADDR<<1, BMP280_REG_ID, 1, &check, 1, TIMEOUT_I2C)) return HAL_ERROR;
	if(check != BMP280_CHIP_ID) return false;

	/* reset the chip */
	if(HAL_I2C_Mem_Write(BMP280_I2C_HANDLE, BMP280_ADDR<<1, BMP280_REG_RESET, 1, (uint8_t*)BMP280_RESET_VALUE, 1, TIMEOUT_I2C)) return HAL_ERROR;

	/* Wait until finished copying over the NVP data */
	while (1)
	{
		if(!HAL_I2C_Mem_Read(BMP280_I2C_HANDLE, BMP280_ADDR<<1, BMP280_REG_STATUS, 1, &status, 1, TIMEOUT_I2C) && (status & 1) == 0) break;
	}
	...
	if(HAL_I2C_Mem_Write(BMP280_I2C_HANDLE, BMP280_ADDR<<1, BMP280_REG_CONFIG, 1, &data, 1, TIMEOUT_I2C)) return HAL_ERROR;

	/* Control register */
	data = (BMP280.config.oversampling_temperature << 5 | BMP280.config.oversampling_pressure << 2 | BMP280.config.mode);
	if(HAL_I2C_Mem_Write(BMP280_I2C_HANDLE, BMP280_ADDR<<1, BMP280_REG_CTRL, 1, &data, 1, TIMEOUT_I2C)) return HAL_ERROR;
